...
// Send a 16 bits data to LCD buffer
...
int main(){
    ...
    //gpio 8 sda
    //gpio 9 scl
    //gpio 1 interrupt pin
    pinMode(1, PUD_DOWN);
    wiringPiISR(1,INT_EDGE_BOTH,  display); 
    /*
    Tips:
    ...
    */
	...
	const char text[] = {"Not Pressed"};
	const char text2[] = {"Pressed"};
    while(true) {
        /*You can use write() to write to the LCD.  *	 */
	if(!pressed) write(0,0,text);
	else {
